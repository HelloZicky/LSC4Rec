    ...
    def __getitem__(self, idx):
        ...
        if self.args.framework == "large_language_model":
            if task_name == 'rating':
                ...
                if self.mode == 'train':
                    end_candidates = [_ for _ in range(max(2, len(sequence) - 8), len(sequence) - 5)]
                    ...
                    purchase_history = sequence[start_pos:end_pos]  # sample a history sequence from the full user purchase history
                    target_item = sequence[end_pos]
                elif self.mode == 'val':
                    purchase_history = sequence[1:-5]
                    target_item = sequence[-5]
                elif self.mode == 'test':
                    purchase_history = sequence[1:-4]
            ...
            if task_name == 'sequential' or task_name == 'traditional':
                ...
                if self.mode == 'train':
                    if self.args.dcc == 'True':
                        ...
                        start_pos = start_candidates[start_index]
                        purchase_history = sequence[start_pos:end_pos]  # sample a history sequence from the full user purchase history
                        trigger_purchase_history = sequence[max(0,start_pos - 1):end_pos-1] 
                        target_item = sequence[end_pos]
                    else:
                    ...
                    if self.args.dcc == 'True':
                        purchase_history = sequence[1:-4]
                        trigger_purchase_history = sequence[1:-5]  # sample a history sequence from the full user purchase history
                        target_item = sequence[-4]
                    else:
                        purchase_history = sequence[1:-2]
                        trigger_purchase_history = sequence[1:-3]  # sample a history sequence from the full user purchase history
                        target_item = sequence[-2]
                elif self.mode == 'test':
                    if self.args.dcc == 'True':
                        purchase_history = sequence[1:-3]
                        trigger_purchase_history = sequence[1:-4]  # sample a history sequence from the full user purchase history
                        target_item = sequence[-3]
                    else:
                        purchase_history = sequence[1:-1]
                        trigger_purchase_history = sequence[1:-2]  # sample a history sequence from the full user purchase history
                        target_item = sequence[-1]
                else:
                    raise NotImplementedError
            ...
            if task_name == 'sequential':
                ...
                if self.mode == 'train':
                    ...
                    start_index = random.randint(0, len(start_candidates) - 1)
                    start_pos = start_candidates[start_index]
                    purchase_history = sequence[start_pos:end_pos] 
                    trigger_purchase_history = sequence[max(0,start_pos - 1):end_pos]
                    target_item = sequence[end_pos]
                    ...
                    purchase_history = sequence[1:-5]
                    trigger_purchase_history = sequence[1:-6]
                    target_item = sequence[-5]
                elif self.mode == 'test':
                    purchase_history = sequence[1:-4]
                    trigger_purchase_history = sequence[1:-5]
                    target_item = sequence[-1]
                else:
                    raise NotImplementedError
        ...
        if self.args.framework == "large_language_model":
            ...
            if task_name == 'sequential' or task_name == 'traditional':
                ...
                if self.mode == 'train':
                    ...
                    start_pos = start_candidates[start_index]
                    purchase_history = sequence[
                                       start_pos:end_pos]  # sample a history sequence from the full user purchase history
                    trigger_purchase_history = sequence[start_pos:end_pos-1]  # sample a history sequence from the full user purchase history
                    ...
                    trigger_purchase_history = sequence[
                                               1:-6]  # sample a history sequence from the full user purchase history
                    target_item = sequence[-5]
                elif self.mode == 'test':
                    purchase_history = sequence[1:-1]
                    trigger_purchase_history = sequence[
                                               1:-2]  # sample a history sequence from the full user purchase history
                    target_item = sequence[-1]
                else:
                    raise NotImplementedError
