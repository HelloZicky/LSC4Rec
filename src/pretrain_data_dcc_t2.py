...
class P5_Amazon_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys', rating_augment=False, sample_type='random'): 
        ...
        # base_path = '/data/zhantianyu/LLM/P5_DCC_master/'
        ...
        #     for item in items:
        #         item_count[item] += 1
        # self.sequential_data = sequential
        ...
        self.all_item_set = set(self.all_item)
        ...
        for key in list(self.task_list.keys()):
            if key == 'rating':
                ...
                if (sum([0 < int(ind.split('-')[1]) <= 6 or int(ind.split('-')[1]) == 13 for ind in self.task_list[key]])
                        or self.args.framework == "small_recommendation_model_base"
                        or self.args.framework == "small_recommendation_model_duet"):
    ...
    def __getitem__(self, idx):
        ...
        loss_weight = 1.0
        # print("999" * 50)
        # print(self.datum_info)
        # print("888" * 50)
        # print(self.datum_info[idx])
        ...
        if self.args.framework == "large_language_model":
            ...
            input_ids_real = self.tokenizer.encode(
                    source_text_real, padding=True, truncation=True, max_length=self.args.max_text_length) #
            tokenized_text = self.tokenizer.tokenize(source_text)
            tokenized_text_real = self.tokenizer.tokenize(source_text_real) #
            ...
            out_dict['input_ids_real'] = torch.LongTensor(input_ids_real) #
            ...
            out_dict['source_text_real'] = source_text_real #
            out_dict['tokenized_text'] = tokenized_text
            out_dict['tokenized_text_real'] = tokenized_text_real #
            ...
            out_dict['mode'] = "large_language_model"

            return out_dict

        elif self.args.framework == "small_recommendation_model_base" or self.args.framework == "small_recommendation_model_duet":
            history_limit = 10
            # if task_name == 'Sequential_ctr':
            if task_name == 'sequential' or task_name == 'traditional':
                if task_name == 'sequential':
                    self.args.full_ranking = True
                    # self.args.full_ranking = False
                    candidate_num = 4
                    # candidate_num = 1
                elif task_name == 'traditional':
                    # self.args.full_ranking = True
                ...
                if self.mode == 'train':
                    ...
                    trigger_purchase_history = sequence[max(0,
                                                            start_pos - 1):end_pos]  # sample a history sequence from the full user purchase history

                    target_item = sequence[end_pos + 1]
                elif self.mode == 'val':
                    purchase_history = sequence[1:-2]
                    trigger_purchase_history = sequence[
                                               0:-3]  # sample a history sequence from the full user purchase history
                    target_item = sequence[-2]
                elif self.mode == 'test':
                    purchase_history = sequence[1:-1]
                    trigger_purchase_history = sequence[
                                               0:-2]  # sample a history sequence from the full user purchase history
                    target_item = sequence[-1]
                else:
                    raise NotImplementedError
                ...
                if self.mode in ['train', 'val']:
                    ...
                    assert user_id == self.negative_samples[int(user_id)-1].split(' ', 1)[0]
                    if self.args.full_ranking:
                        # print("---" * 50)
                        # print(self.all_item)
                        # print(target_item)
                        # print(type(self.all_item[0]))  # int
                        # print(type(target_item))  # str
                        # candidate_samples = self.all_item.remove(target_item)
                        # candidate_samples = self.all_item.remove(int(target_item))
                        # print("---" * 20)
                        # print(len(self.all_item))
                        # print(len(self.all_item_set))
                        target_item_list = [int(target_item)]
                        candidate_samples = self.all_item_set - set(target_item_list)
                        # print(len(candidate_samples))
                    elif not self.args.full_ranking:
                        candidate_samples = self.negative_samples[int(user_id)-1].split(' ', 1)[1].split(' ')
                else:
                    raise NotImplementedError
                ...
                elif len(purchase_history) < history_limit:
                    for i in range(history_limit - len(purchase_history)):
                        purchase_history.append("0")

                if len(trigger_purchase_history) > history_limit:
                    trigger_purchase_history = trigger_purchase_history[-history_limit:]
                elif len(trigger_purchase_history) < history_limit:
                    for i in range(history_limit - len(trigger_purchase_history)):
                        trigger_purchase_history.append("0")

                purchase_history_str = ",".join(purchase_history)
                trigger_purchase_history_str = ",".join(trigger_purchase_history)

                out_dict["candidate_samples"] = candidate_samples
                candidate_samples_num = len(candidate_samples)
                out_dict[consts.FIELD_USER_ID] = [int(user_id) for i in range(candidate_samples_num + 1)]
                out_dict[consts.FIELD_TARGET_ID] = [int(target_item)] + [int(candidate_sample) for candidate_sample in candidate_samples]

                out_dict[consts.FIELD_CLK_SEQUENCE] = np.fromstring(purchase_history_str, dtype=np.int32,
                                                                    sep=",")[:, np.newaxis].repeat(candidate_samples_num + 1, 1)
                out_dict[consts.FIELD_CLK_SEQUENCE] = np.swapaxes(out_dict[consts.FIELD_CLK_SEQUENCE], 0, 1)

                out_dict[consts.FIELD_TRIGGER_SEQUENCE] = np.fromstring(trigger_purchase_history_str,
                                                                        dtype=np.int32, sep=",")[:, np.newaxis].repeat(candidate_samples_num + 1, 1)
                out_dict[consts.FIELD_TRIGGER_SEQUENCE] = np.swapaxes(out_dict[consts.FIELD_TRIGGER_SEQUENCE], 0, 1)

                out_dict[consts.FIELD_LABEL] = [1.0] + [0.0 for i in range(candidate_samples_num)]

                out_dict['task'] = task_name  # Sequential_ctr
                out_dict['framework'] = self.args.framework  # "small_recommendation_model"

            elif task_name == 'Title_ctr':
                out_dict['task'] = task_template['task']  # Sequential_ctr
                out_dict['framework'] = self.args.framework  # "small_recommendation_model"
                pass
            elif task_name == 'Sequential_Title_ctr':
                out_dict['task'] = task_template['task']  # Sequential_ctr
                out_dict['framework'] = self.args.framework  # "small_recommendation_model"
                pass
            elif task_name == 'Sequential_Title_title':
                out_dict['task'] = task_template['task']  # Sequential_ctr
                out_dict['framework'] = self.args.framework  # "small_recommendation_model"
                pass
            elif task_name == 'Sequential_Title_title_ctr':
                out_dict['task'] = task_template['task']  # Sequential_ctr
                out_dict['framework'] = self.args.framework  # "small_recommendation_model"
                pass
            elif task_name == 'Title_ctr':
            ...
            if task_name == 'sequential':
                ...
                if self.mode == 'train':
                    ...
                    start_pos = start_candidates[start_index]
                    purchase_history = sequence[start_pos:end_pos + 1]  # sample a history sequence from the full user purchase history
                    trigger_purchase_history = sequence[max(0, start_pos - 1):end_pos]
                    target_item = sequence[end_pos + 1]
                elif self.mode == 'val':
                    purchase_history = sequence[1:-2]
                    trigger_purchase_history = sequence[1:-3]
                    target_item = sequence[-2]
                elif self.mode == 'test':
                    purchase_history = sequence[1:-1]
                    # trigger_purchase_history = sequence[1:-2]
                    # trigger_purchase_history = sequence[1:-4]
                    trigger_purchase_history = sequence[1:-3] #dcc_t2
                    target_item = sequence[-1]
                else:
                    raise NotImplementedError
                ...
                if task_template['id'] == '2-3':
                    rand_prob = random.random()
                    if rand_prob > 0.5:
                        # if (not self.args.dcc) or (self.args.dcc and self.mode != 'test'):
                        if not self.args.dcc:
                            source_text = task_template['source'].format(user_id, ' , '.join(purchase_history))
                        # elif self.args.dcc and self.mode == 'test':
                        elif self.args.dcc:
                            source_text = task_template['source'].format(user_id, ' , '.join(trigger_purchase_history))
                    else:
                        # if (not self.args.dcc) or (self.args.dcc and self.mode != 'test'):
                        if not self.args.dcc:
                            source_text = task_template['source'].format(user_id, ' -> '.join(purchase_history))
                    ...
                    if rand_prob > 0.5:
                        ...
                        # elif self.args.dcc and self.mode == 'test':
                            ...
                            source_text = task_template['source'].format(user_id, ' , '.join(trigger_purchase_history))
                            source_text_real = task_template['source'].format(user_id, ' , '.join(purchase_history))
                    else:
                        # if (not self.args.dcc) or (self.args.dcc and self.mode != 'test'):
                        if not self.args.dcc:
                            source_text = task_template['source'].format(user_id, ' -> '.join(purchase_history))
                ...
                if self.mode == 'train':
                    ...
                    target_item = target_candidates[target_idx]
                    purchase_history = target_candidates[:target_idx] # collaboration
                    trigger_purchase_history = target_candidates[:max(0, target_idx - 1)]
                elif self.mode == 'val':
                    target_item = sequence[-2]
                    purchase_history = sequence[1:-2]  # collaboration
                    trigger_purchase_history = sequence[0:-3]
                elif self.mode == 'test':
                    target_item = sequence[-1]
                    purchase_history = sequence[1:-1]  # collaboration
                    trigger_purchase_history = sequence[0:-2]
                else:
                    raise NotImplementedError
            ...
            out_dict['mode'] = "collaboration"

            if self.args.dcc:
                out_dict['input_ids_real'] = torch.LongTensor(input_ids_real)
                out_dict['input_length_real'] = len(input_ids_real)
                out_dict['source_text_real'] = source_text_real #
                out_dict['tokenized_text_real'] = tokenized_text_real #
            ### small model
            ...
            if self.mode == 'test' or self.collaboration_method == 2:
                candidate_samples = []

            candidate_samples_num = len(candidate_samples)
            out_dict[consts.FIELD_USER_ID] = [int(user_id) for i in range(candidate_samples_num + 1)]
            out_dict[consts.FIELD_TARGET_ID] = [int(target_item)] + [int(candidate_sample) for candidate_sample in
                                                                     candidate_samples]

            out_dict[consts.FIELD_CLK_SEQUENCE] = np.fromstring(purchase_history_str, dtype=np.int32,
                                                                sep=",")[:, np.newaxis].repeat(
                candidate_samples_num + 1, 1)
            # print("--+--")
            # print(out_dict[consts.FIELD_CLK_SEQUENCE].shape)
            out_dict[consts.FIELD_CLK_SEQUENCE] = np.swapaxes(out_dict[consts.FIELD_CLK_SEQUENCE], 0, 1)
            # print(out_dict[consts.FIELD_CLK_SEQUENCE].shape)
            out_dict[consts.FIELD_TRIGGER_SEQUENCE] = np.fromstring(trigger_purchase_history_str,
                                                                    dtype=np.int32, sep=",")[:, np.newaxis].repeat(
                candidate_samples_num + 1, 1)
            # print(out_dict[consts.FIELD_TRIGGER_SEQUENCE].shape)
            out_dict[consts.FIELD_TRIGGER_SEQUENCE] = np.swapaxes(out_dict[consts.FIELD_TRIGGER_SEQUENCE], 0, 1)
            # print(out_dict[consts.FIELD_TRIGGER_SEQUENCE].shape)
            out_dict[consts.FIELD_LABEL] = [1.0] + [0.0 for i in range(candidate_samples_num)]
            # print("++++++++++" * 10)
    ...
    def collate_fn(self, batch):
        if self.args.framework == "large_language_model":
            ...
            target_id_list = []
            click_seq_list = []
            trigger_click_seq_list = []
            label_list = []

            for i, entry in enumerate(batch):
                user_id_list.extend(entry[consts.FIELD_USER_ID])
                target_id_list.extend(entry[consts.FIELD_TARGET_ID])
                click_seq_list.extend(entry[consts.FIELD_CLK_SEQUENCE])
                trigger_click_seq_list.extend(entry[consts.FIELD_TRIGGER_SEQUENCE])
                label_list.extend(entry[consts.FIELD_LABEL])

            batch_entry[consts.FIELD_USER_ID] = torch.from_numpy(np.array(user_id_list))
            batch_entry[consts.FIELD_TARGET_ID] = torch.from_numpy(np.array(target_id_list))
            batch_entry[consts.FIELD_CLK_SEQUENCE] = torch.from_numpy(np.array(click_seq_list))
            batch_entry[consts.FIELD_TRIGGER_SEQUENCE] = torch.from_numpy(np.array(trigger_click_seq_list))
            batch_entry[consts.FIELD_LABEL] = torch.from_numpy(np.array(label_list))
            # print("----")
            # print(batch_entry[consts.FIELD_USER_ID].size())
            # print(batch_entry[consts.FIELD_TARGET_ID].size())
            # print(batch_entry[consts.FIELD_CLK_SEQUENCE].size())
            # print(batch_entry[consts.FIELD_TRIGGER_SEQUENCE].size())
            # print(batch_entry[consts.FIELD_LABEL].size())
            return batch_entry
            ...
            input_ids = torch.ones(B, S_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
            if self.args.dcc:
                input_ids_real = torch.ones(B, max(entry['input_length_real'] for entry in batch), dtype=torch.long) * self.tokenizer.pad_token_id
            whole_word_ids = torch.ones(B, S_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
            target_ids = torch.ones(B, T_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
            ...
            for i, entry in enumerate(batch):
                ...
                if 'input_ids_real' in entry:
                    input_ids_real[i, :entry['input_length_real']] = entry['input_ids_real']
                    
                if 'source_text_real' in entry:
                    source_text_real.append(entry['source_text_real'])
            ...
            batch_entry['loss_weights'] = loss_weights
            if self.args.dcc:
                batch_entry['source_text_real'] = source_text_real
                batch_entry['input_ids_real'] = input_ids_real
            # print("np.array(user_id_list).shape ", np.array(user_id_list).shape)
            # print("np.array(target_id_list).shape ", np.array(target_id_list).shape)
            # print("np.array(click_seq_list).shape ", np.array(click_seq_list).shape)
            # print("np.array(trigger_click_seq_list).shape ", np.array(trigger_click_seq_list).shape)
            # print("np.array(label_list).shape ", np.array(label_list).shape)
            batch_entry[consts.FIELD_USER_ID] = torch.from_numpy(np.array(user_id_list))
            batch_entry[consts.FIELD_TARGET_ID] = torch.from_numpy(np.array(target_id_list))
            batch_entry[consts.FIELD_CLK_SEQUENCE] = torch.from_numpy(np.array(click_seq_list))
            batch_entry[consts.FIELD_TRIGGER_SEQUENCE] = torch.from_numpy(np.array(trigger_click_seq_list))
    ...
    def __getitem__(self, idx):
        ...
        if self.args.framework == "large_language_model":
            ...
            if task_name == 'sequential' or task_name == 'traditional':
                ...
                out_dict[consts.FIELD_TRIGGER_SEQUENCE] = np.fromstring(trigger_purchase_history_str, dtype=np.int32,
                                                                        sep=",")[:, np.newaxis].repeat(candidate_samples_num + 1, 1)
                out_dict[consts.FIELD_TRIGGER_SEQUENCE] = np.swapaxes(out_dict[consts.FIELD_TRIGGER_SEQUENCE], 0, 1)
        ...
        # elif self.args.framework == "collaboration":
            ...
            if task_name == 'sequential':
                ...
                if self.mode == 'train':
                    ...
                    purchase_history = sequence[1:-1]
                    # trigger_purchase_history = sequence[0:-2]
                    # trigger_purchase_history = sequence[0:-4]
                    trigger_purchase_history = sequence[1:-3]
                    target_item = sequence[-1]
                else:
                    raise NotImplementedError
...
def get_loader(args, task_list, sample_numbers, split='toys', mode='train',
    ...
    if args.framework == "large_language_model" or args.framework == "collaboration":
